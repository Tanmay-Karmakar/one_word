...
def load_queries(tokenize_dir, mode):
    queries = dict()
    for line in tqdm(open(f"{tokenize_dir}"), desc="queries"): #/msmarco-passage{mode}-queries.tokenized.json
        data = json.loads(line)
        if int(data['id']) == 156493:
            print(f"** {data['id']}")
...
def load_querydoc_pairs_by_file_path(qd_file_path, mode):
    qids, pids, labels = [], [], []
    # file_format: qid\tpassid\tscore\trank
    fd = open(qd_file_path)
    fd.readline()
    for line in fd:
