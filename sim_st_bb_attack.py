...
def attack_qc_sim_query(args, qid, query_center, attack_pass_qid, orig_pass_qid, query, tokenizer, model_ref, model_name):
    ...
    for j in range(len(attack_pass_qid)):
        ...
        qc_sim_indices = find_qc_sim_indices(query_center, attack_pass_tokens)
        ...
        to_write = f"{qid}\t{pid}\t{orig_pass_len}\t{attack_num}\t{max_score}\t[DIV]\t"+attack_pass
...
def run_parse_args():
    ...
    # parser.add_argument("--orig_pass_file", type=str, default="./reranked_files/ori_pass_files/monoT5_trec_dl_2019")
    
    #------------------------------------------------------------------------
    # monoT5 2019
    # python3 sim_st_bb_attack.py --model_name monoT5 --attack_dataset trec_dl_2019 --save_file_name sim_ow_01 --orig_pass_file ./reranked_files/ori_pass_files/monoT5_trec_dl_2019
    
    # monoT5 2020
    # python3 sim_st_bb_attack.py --model_name monoT5 --attack_dataset trec_dl_2020 --save_file_name sim_ow_01 --orig_pass_file ./reranked_files/ori_pass_files/monoT5_trec_dl_2020
    
    # Distilbert 2019
    # python3 sim_st_bb_attack.py --model_name Distilbert --attack_dataset trec_dl_2019 --save_file_name sim_ow_01 --orig_pass_file ./reranked_files/ori_pass_files/Distilbert_trec_dl_2019

    # Distilbert 2020 
    # python3 sim_st_bb_attack.py --model_name Distilbert --attack_dataset trec_dl_2020 --save_file_name sim_ow_01 --orig_pass_file ./reranked_files/ori_pass_files/Distilbert_trec_dl_2020
...
def main():
    ...
    #     qid = query_file['qid'][i]
    #     query = query_file['query'][i]
    #     orig_pass_qid = orig_pass[orig_pass['qid']==qid]
    #     attack_pass_qid = attack_qp_pass[attack_qp_pass['qid']==qid]
    #     query_center, simm = get_query_center(query, args.embed_path, bert_tokenizer)
    #     print(f"*** query = {query}")
    #     print(f"*** query_center = {query_center}")
    ...
    #         attack_pass = orig_pass_qid[orig_pass_qid['pid'] == pid]['pass'].iloc[0]
            
    #         orig_score = word_re.get_score_model(query, attack_pass, model_ref, model_name)
    #         max_score = orig_score
            
    #         attack_pass_tokens = bert_tokenizer.tokenize(attack_pass)
    #         qc_tokens = bert_tokenizer.tokenize(query_center)
    #         qc_sim_indices = find_qc_sim_indices(query_center, attack_pass_tokens)
    #         # attacked_pass = attack_pass
            
    #         k = 0
    #         attack_num = 0
    #         while(k<args.max_attack_word_number):
    #             new_pass_tokens = replace_one_word_by_qc(qc_tokens, attack_pass_tokens, qc_sim_indices, k)
    #             new_pass = bert_tokenizer.convert_tokens_to_string(new_pass_tokens)
    #             new_score = word_re.get_score_model(query, new_pass, model_ref, model_name)
    #             if new_score > max_score:
    #                 max_score = new_score
    #                 attack_pass = new_pass
    #                 attack_pass_tokens = new_pass_tokens
