...
from transformers.models.t5.modeling_t5 import (T5Model, T5PreTrainedModel) #done
logger = logging.getLogger(__name__)


class RankingBERT_Train(BertPreTrainedModel):  #done
    def __init__(self, config):
        super(RankingBERT_Train, self).__init__(config)
        self.bert = BertModel(config)  
        self.init_weights()

        self.dropout = nn.Dropout(p=config.hidden_dropout_prob) # p=config.hidden_dropout_prob done
        self.out = nn.Linear(config.hidden_size,1) # config.hidden_size done
