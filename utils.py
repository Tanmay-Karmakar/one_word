...
def read_qd_pairs(QP_FILE_PATH):
    qds = {}
    with open(QP_FILE_PATH, 'r', encoding='utf-8') as qdsf:
        for line in qdsf:
            ss = line.strip().split('\t')
            qid = int(ss[0])
            did = int(ss[1])
...
def read_qps_pairs(QP_FILE_PATH):     
    qds = {}
    with open(QP_FILE_PATH, 'r', encoding='utf-8') as qpsf:
        qpsf.readline()
        for line in qpsf:
            ss = line.strip().split()
